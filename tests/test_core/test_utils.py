...
def test_sanitize_filename_fixed():
    ...
    test_cases = [
        ...
        # FIX: transliteration lowercases each char before lookup, so result is lowercase
        ("ВОПРОС.docx", "vopros.docx"),
        ("  trimmed  .pdf", "trimmed_.pdf"),
        ("", "unnamed_file"),  # FIX: utils.py returns "unnamed_file", not "unknown_file"
    ]
...
def test_sanitize_filename_edge_cases():
    ...
    # FIX: utils.py returns "unnamed_file", not "unknown_file"
    ...
    # "***.txt" → "___.txt" → "_.txt" (collapse) → "txt" (strip leading _)
    assert sanitize_filename("***.txt") == "txt"

    # Тест с путем
    assert sanitize_filename("/path/to/file.txt") == "file.txt"
    # On Linux, Path("C:\\Windows\\file.txt").name is the whole string (no path stripping)
    assert sanitize_filename("C:\\Windows\\file.txt") == "C_Windows_file.txt"

    # Тест с только подчеркиваниями
    # "___.txt" → "_.txt" (collapse) → "txt" (strip leading _)
    assert sanitize_filename("___.txt") == "txt"
    # "__" → "" after stripping → hits uuid fallback, not "unnamed_file"
    assert sanitize_filename("__").startswith("file_")
...
class TestCalculateHash:
    ...
    def test_calculate_hash_nonexistent_file(self):
        """
        FIX: calculate_hash() returns an error string instead of raising
        FileNotFoundError — check for the error string instead.
        """
        result = calculate_hash(Path("/nonexistent/path/file.txt"))
        assert result.startswith("hash_error:")
