...
## 13. Профили развёртывания (DEPLOYMENT_TYPE)

Одна кодовая база обслуживает несколько сценариев эксплуатации. Активный
профиль выбирается переменной окружения **`DEPLOYMENT_TYPE`**, а конкретный
набор включённых возможностей задаётся матрицей `FEATURE_MATRIX` в
`app/core/feature_flags.py`. Так одни и те же модули API, хранилища и аудита
работают и в строго регулируемой среде (ФЗ-152), и в публичном демо без
дублирования кода.

### 13.1 Доступные профили

| Профиль  | Назначение                                                                                              |
|----------|---------------------------------------------------------------------------------------------------------|
| `russia` | ФЗ-152: только локальное хранилище, обязательная 2FA, аудит 3 года (1095 дней), задел под ГОСТ/ГосСОПКА  |
| `intl`   | Международная установка: S3/MinIO, DICOM, функции GDPR/HIPAA, опциональная 2FA, PACS                     |
| `single` | Один арендатор: упрощённая админка, локальный диск по умолчанию, авто-SSL и авто-бэкап                   |
| `saas`   | Мультиарендность: биллинг и white-label, объектное хранилище, обязательная 2FA для администраторов       |
| `demo`   | Публичная демонстрация: локальное хранилище, опциональная 2FA, малый лимит загрузки, автосброс данных    |

### 13.2 Как это работает

- Матрица `FEATURE_MATRIX` сопоставляет каждому профилю словарь
  `Feature -> bool`. Проверка выполняется через `feature_flags.is_enabled(...)`.
- 2FA имеет отдельную логику: `is_2fa_mandatory()` и
  `is_2fa_required_for_user(role)` — например, в `saas` 2FA обязательна только
  для ролей `admin` и `super_admin`, а в `russia` — для всех.
- Срок хранения аудита (`audit_retention_days`) и лимиты (загрузка, частота
  запросов) подстраиваются под профиль и переопределяются переменными
  окружения (см. [`API_GUIDE.md`](API_GUIDE.md), раздел Rate Limiting).

### 13.3 Профиль `demo`

Профиль `demo` рассчитан на публичный показ возможностей без рисков для
реальных данных:

- только локальное хранилище (без MinIO/S3), чтобы укладываться в минимальные
  ресурсы;
- 2FA доступна, но не обязательна — чтобы не мешать быстрому ознакомлению;
- включены DICOM Viewer и функции GDPR/HIPAA (право на забвение, переносимость
  данных) как витрина соответствия;
- сниженный лимит размера загружаемого файла;
- автоматический сброс и пересев демо-данных каждые `DEMO_RESET_INTERVAL_HOURS`
  часов (по умолчанию 24);
- публичный эндпоинт `GET /api/demo/info` отдаёт демо-учётки, список фич и время
  до следующего сброса; роутер монтируется только при `DEMO_MODE=true`.

См. также `.env.demo.example` и `docker-compose.demo.yml`.

### 13.4 Проверка профиля в рантайме

- HTTP: `GET /health/features` (карта фич) и `GET /health/deployment` (сводка по
  профилю, включая `audit_retention_days` и `mandatory_2fa_for_all`).
- CLI: `python -m app.cli feature-info` (сводка) и
  `python -m app.cli feature-check <feature>` (проверка одного флага).
