...
3. [Multi-tenant контекст](#3-multi-tenant-контекст-host--jwt)
4. [Форматы данных и ограничения](#4-форматы-данных-и-ограничения)
5. [Обработка ошибок](#5-обработка-ошибок)
6. [Rate Limiting](#6-rate-limiting)
7. [Бизнес-правила и особенности](#7-бизнес-правила-и-особенности)
...
Роль	    Права
super_admin	Как у admin; не блокируется перекрёстным доступом между tenant (см. раздел 3)
...
## 3. Multi-tenant контекст (Host + JWT)

Экземпляр может обслуживать несколько организаций (**tenants**). Отдельного заголовка вида `X-Tenant-Id` API **не использует**.

### 3.1. Как передать tenant

| Механизм | Описание |
|----------|----------|
| **`Host`** | Основной способ. Первый лейбл поддомена сопоставляется с полем `tenants.subdomain` (например `clinic.example.com` → `clinic`). Для локальной разработки без поддомена см. переменные `TENANT_DEFAULT_SUBDOMAIN` и `TENANT_RESOLVE_LOCALHOST_AS_DEFAULT`. |
| **JWT (`tenant_id`)** | После `POST /api/auth/login` в cookie/Bearer попадает токен с полем `tenant_id`. Оно должно совпадать с tenant, разрешённым по текущему `Host`, иначе для ролей `admin`/`doctor`/`user` вернётся **403** («Cross-tenant access»). |

Отдельные эндпоинты вызывают внутри себя `require_tenant(request)` — если поддомен не найден в БД, ответ **400** («Tenant could not be resolved from subdomain»).

### 3.2. Что это значит для клиента

- Открывайте UI и вызывайте API по **каноническому хосту организации** (поддомен в DNS).
- Интеграции (`curl`, Postman, сервисы) должны отправлять корректный **`Host`** (или использовать реальный DNS поддомена).
- Нельзя взять JWT, выданный для `tenant A`, и вызывать API с `Host`, соответствующим `tenant B` (кроме роли `super_admin`).

Подробности: [docs/MULTI_TENANCY.md](docs/MULTI_TENANCY.md).

---

## 4. Форматы данных и ограничения

### 4.1. Поля с особыми правилами
...
### 5.1. Структура ответа с ошибкой
...
## 7. Бизнес-правила и особенности

### 7.1. Работа с пользователями (Admin API)
...
### 8.2. Скачивание по одноразовой ссылке
